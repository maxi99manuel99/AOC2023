...
class HeatLossMap():
    ...
    def find_route_with_min_heat_loss(self, start_block: tuple[int, int], goal_block: tuple[int, int]) -> int:
        ...
        for neighbor_pos, (neighbor_direction, neighbor_streak) in self.get_neighbors(start_block, DIRECTION.UNDEFINED, 0):
            neighbor_obj = QueueObject(neighbor_pos, self.heat_map[neighbor_pos], (neighbor_direction, neighbor_streak))
        ...
        while priority_queue:
            ...
            for neighbor_pos, (neighbor_direction, neighbor_streak) in self.get_neighbors(pos, prev_direction, direction_streak):
                if (neighbor_pos, neighbor_direction, neighbor_streak) not in visited:
                    neighbor_obj = QueueObject(neighbor_pos, heat_loss + self.heat_map[neighbor_pos], (neighbor_direction, neighbor_streak))
                    heapq.heappush(priority_queue, neighbor_obj)
