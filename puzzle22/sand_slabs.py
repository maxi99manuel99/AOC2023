...
class BrickMap():
    ...
    def bricks_fall(self) -> None:
        ...
        for brick_key, brick in self.bricks:
            ...
            brick.end = (brick.end[0], brick.end[1], lowest_z_possible+(brick.end[2] - brick.start[2]))
            brick.start = (brick.start[0], brick.start[1], lowest_z_possible)

            self.update_brick_map(brick_key, old_brick_start, old_brick_end, brick.start, brick.end, brick.expanding_dim)
...
if __name__ == "__main__":
    ...
    with open("input.txt") as fp:
        ...
        for j, (brick_start, brick_end) in enumerate(bricks):
            expanding_dim = np.where(np.array(brick_start) != np.array(brick_end))[0]
            if len(expanding_dim) > 0:
                expanding_dim = expanding_dim[0]
                bricks[j] = (j+1, BrickRange(expanding_dim=expanding_dim, start=brick_start, end=brick_end))
                ...
                bricks[j] = (j+1, BrickRange(expanding_dim=np.inf, start=brick_start, end=brick_end))
